//Mon Dec 23 2024 15:07:47 GMT+0000 (Coordinated Universal Time)
    ...
    case /^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\?/.test($request.url):
      {
        try {
          ...
          for (let o of s.data.items) if (!o.hasOwnProperty("banner_item")) {
            if (!(!o.hasOwnProperty("ad_info") && -1 === o.card_goto?.indexOf("ad") && ["small_cover_v2", "large_cover_v1", "large_cover_single_v9"].includes(o.card_type))) {
              continue;
            } else {
              l.push(o);
            }
    ...
    case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test($request.url):
      {
        try {
          ...
          if (h.data?.tab) {
            ...
            if (-1 == JSON.stringify(h.data.tab).indexOf("pgc/home")) {
              f.push({
                id: 774,
                name: "番剧",
    ...
    case /^https:\/\/api\.live\.bilibili\.com\/xlive\/app-interface\/v2\/index\/feed/.test($request.url):
      {
        try {
          let A = JSON.parse(body);
          A.data && A.data.card_list && (A.data.card_list = A.data.card_list.filter(t => "banner_v1" != t.card_type));
          body = JSON.stringify(A);
        } catch (C) {
